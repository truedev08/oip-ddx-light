...
class Artifact extends _oipRecord.default {
  constructor(input) {
    super(input);
    this.artifact = {
      floAddress: '',
    ...
    if (input) {
      if (typeof input === 'string') {
        ...
        try {
          ...
          console.error('Error parsing input in Artifact constructor', e);
        }
  ...
  setTXID(txid) {
    ...
    if (!this.getOriginalTXID) {
      this.setOriginalTXID(this.getTXID);
    }
  }
  /**
   * Get TXID
  ...
  getPaymentAddress(coins) {
    ...
    if (Array.isArray(coins)) {
      for (const coin of coins) {
        for (const _coin in this.artifact.payment.addresses) {
          if (coin === _coin) {
  ...
  getSupportedCoins(coins) {
    const coinCheck = coins || undefined;
    const supportedCoins = [];
    const addrs = this.getPaymentAddresses();
    ...
    if (coinCheck) {
      if (Array.isArray(coinCheck)) {
        ...
        for (const myCoin of coinCheck) {
          for (const supCoin of supportedCoins) {
            if (myCoin === supCoin) {
              _coins.push(myCoin);
            }
          }
  ...
  addFile(file) {
    if (file instanceof _files.ArtifactFile) {
      this.FileObjects.push(new _files.ArtifactFile(file.toJSON(), this));
    } else if (typeof file === 'object' || typeof file === 'string') {
      this.FileObjects.push(new _files.ArtifactFile(file, this));
    }
  }
  /**
  ...
  isValid() {
    if (!this.artifact.info.title || this.artifact.info.title === '') {
      return {
        success: false,
        error: 'Artifact Title is a Required Field'
      };
    }
  ...
  toJSON() {
    ...
    const cloneJSON = JSON.parse(JSON.stringify(artJSON)); // OIPd will not accept artifacts that have storage defined without any files
      ...
      if (!artifact.meta) {
        ...
        switch (artifact.meta.type) {
          ...
          default:
            return {
              success: false,
              error: 'Unsupported media type. Check artifact.meta.type',
              detail: artifact
            };
        }
      }
  ...
  importAlexandriaMedia(artifact) {
    ...
    if (artifact.info) {
      ...
      if (artifact.info['extra-info']) {
        ...
        for (const key in artifact.info['extra-info']) {
          if (artifact.info['extra-info'].hasOwnProperty(key)) {
            switch (key) {
              ...
              case 'DHT Hash':
                const hash = artifact.info['extra-info'][key];
              ...
              case 'files':
                const fileList = artifact.info['extra-info'][key];
  ...
  import041(artifact) {
    ...
    if (artifact.type) {
      if (artifact.type.split('-').length === 2) {
        const type = artifact.type.split('-')[0];
        const subtype = artifact.type.split('-')[1];
        this.setType(type);
    ...
    if (artifact.info) {
      ...
      if (artifact.info.extraInfo) {
        for (const key in artifact.info.extraInfo) {
    ...
    if (artifact.storage) {
      ...
      if (artifact.storage.files) {
        for (const file of artifact.storage.files) {
          this.addFile(file);
        }
      }
    }
    ...
    if (artifact.payment) {
      ...
      if (artifact.payment.tokens && Array.isArray(artifact.payment.tokens)) {
        for (const token of artifact.payment.tokens) {
          this.addTokenRule(token);
        }
      }

      if (artifact.payment.addresses) {
        for (const address of artifact.payment.addresses) {
  ...
  import042(artifact) {
    ...
    if (artifact.details) {
      for (const key in artifact.details) {
    ...
    if (artifact.payment) {
      ...
      if (artifact.payment.addresses) {
        for (const coin in artifact.payment.addresses) {
  ...
  createPreimage() {
    ...
    const preimage = `${this.getLocation() || ''}-${this.getPubAddress()}-${this.getTimestamp()}`;
  ...
  serialize(method) {
    ...
    const artJSON = this.toJSON();
    const art = artJSON.artifact; // setup initial object

    const pubMessage = {
      oip042: {} // insert method type

    };
    pubMessage.oip042[method] = {
      artifact: art // add json prefix
