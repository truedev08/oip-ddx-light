...
class EditRecord extends _oipRecord.default {
  ...
  async signSelf(signMessage) {
    ...
    if (this.getPatchedRecord()) {
      ...
      const _this$getPatchedRecor = this.getPatchedRecord().isValid(),
  ...
  setPatch(rfc6902Patch) {
    ...
    if (this.originalRecord && !this.patchedRecord) {
      const patchedRecord = this.createPatchedRecord(this.originalRecord, this.edit.patch);
      this.setPatchedRecord(patchedRecord);
    }
  }
  ...
  createPatchedRecord(originalRecord, rfc6902Patch) {
    const clonedJSON = originalRecord.toJSON();
    const patchOperations = (0, _rfc.applyPatch)(clonedJSON.artifact, rfc6902Patch);
    ...
    const patchedRecord = (0, _decoders.decodeArtifact)(clonedJSON);
    this.setPatchedRecord(patchedRecord);
  ...
  createPatch() {
    ...
    const originalJSON = this.originalRecord.toJSON().artifact;
    const patchedJSON = this.patchedRecord.toJSON().artifact;
  ...
  toJSON() {
    const cloneJSON = JSON.parse(JSON.stringify({
      edit: this.edit,
      meta: this.meta
    }));
    return cloneJSON;
  ...
  isValid() {
    ...
    for (const operation of this.edit.patch) {
