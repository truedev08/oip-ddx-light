...
class FLOTransaction extends _bitcoinjsLib.Transaction {
  ...
  setFloData(floData, dataType) {
    const tmpFloData = Buffer.from(floData, dataType); // Check if the floData is too long :)
  ...
  __byteLength(__allowWitness, options) {
    const byteLength = _bitcoinjsLib.Transaction.prototype.__byteLength.call(this, __allowWitness);

    if (options && options.excludeFloData || this.version < 2) {
      return byteLength;
    }

    const floDataVarInt = _varuintBitcoin.default.encode(this.floData.length);
  ...
  hashForSignature(inIndex, prevOutScript, hashType, options) {
    ...
    const ourScript = _bitcoinjsLib.script.compile(_bitcoinjsLib.script.decompile(prevOutScript).filter(x => {
    ...
    const buffer = Buffer.alloc(txTmp.__byteLength(false, options) + 4);
