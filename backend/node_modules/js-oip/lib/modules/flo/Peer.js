...
class Peer {
  ...
  constructor(settings) {
    ...
    this.settings = settings; // Rename the mainnet network to be what fcoin expects

    if (this.settings.network === 'mainnet' || this.settings.network === 'livenet') {
      this.settings.network = 'main';
    } // Set the initial connection status


    this.connected = false; // Initialize the transaction map to store { 'txid': 'hex' } the txid related to the hex

    this.txMap = {}; // Initialize a logging limiter variable
  ...
  async connect() {
    const log = (0, _fcoin.logger)({
      level: 'spam'
    }); // Create the Fcoin Peer

    this.peer = _fcoin.peer.fromOptions({
      logger: log,
    ...
    const address = _fcoin.netaddress.fromHostname(this.settings.ip); // Fire off the initial connection attempt


    this.peer.connect(address); // When we recieve packets from peers, process them using the onPacket function

    this.peer.on('packet', this.onPacket.bind(this)); // Handle/log errors
    ...
    try {
      ...
      await this.peer.open(); // Set connected to true once we have connected
  ...
  announceTX(hex) {
    ...
    if (this.connected) {
      try {
        ...
        const mytx = (0, _fcoin.tx)().fromRaw(Buffer.from(hex, 'hex')); // Store the hash of the transaction in our txMap

        this.txMap[mytx.hash('hex')] = hex; // Announce the Transaction to the peer
  ...
  onPacket(packet) {
    ...
    switch (packet.type) {
      ...
      case _fcoin.packets.types.GETDATA:
  ...
  handleGetData(getDataPacket) {
    ...
    let txsRelayed = 0; // Track the last txid hash that we sent

    let lastHash; // If the peer is requesting more than one transaction, or if they have not met the singlelog maximum, then log the request
    ...
    for (const item of getDataPacket.items) {
      // We only care about responding if they are requesting a transction
      const regTX = item.type === _fcoin.invitem.types.TX;
      const segTX = item.type === _fcoin.invitem.types.WITNESS_TX; // If we are a regular tx, or a segwit tx, then continue
      ...
      if (regTX || segTX) {
        ...
        const mytx = (0, _fcoin.tx)().fromRaw(Buffer.from(this.txMap[item.hash], 'hex')); // Include the transaction in a TXPacket to be sent to the Peer

        const txPacket = _fcoin.packets.TXPacket(mytx, segTX); // Send out the TXPacket to the Peer


        this.peer.send(txPacket); // Increase our count of transactions relayed

        txsRelayed++; // Store the previous hash (for logging purposes)
