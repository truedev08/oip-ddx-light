...
var _insightExplorer = _interopRequireDefault(require("insight-explorer"));

var _tx = _interopRequireDefault(require("fcoin/lib/primitives/tx"));
...
class ExplorerWallet {
  ...
  constructor(options) {
    let network = _config.floMainnet;

    if (options.network === 'testnet') {
      network = _config.floTestnet;
    }

    if (options.explorerUrl) {
      network.explorer = new _insightExplorer.default(options.explorerUrl);
    }

    if (!(0, _util.isValidWIF)(options.wif, network.network)) {
      return {
        success: false,
        message: 'Invalid WIF',
        wif: options.wif,
    ...
    this.ECPair = _bitcoinjsLib.ECPair.fromWIF(options.wif, this.network);
    this.p2pkh = _bitcoinjsLib.payments.p2pkh({
  ...
  signMessage(message) {
    const privateKeyBuffer = this.ECPair.privateKey;
    const compressed = this.ECPair.compressed || true;
    let signatureBuffer;

    try {
      signatureBuffer = (0, _bitcoinjsMessage.sign)(message, privateKeyBuffer, compressed, this.ECPair.network.messagePrefix);
    } catch (e) {
      throw new Error(e);
    }
  ...
  async buildTXHex() {
    let floData = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
    ...
    let _selected = selected,
        ...
        outputs = _selected.outputs; // inputs and outputs will be undefined if no solution was found

    if (!inputs || !outputs) {
      throw new Error('No Inputs or Outputs selected! Fail!');
    }

    const txb = new _FLOTransactionBuilder.default(this.network);
  ...
  async buildInputsAndOutputs() {
    let floData = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
    let outputs = arguments.length > 1 ? arguments[1] : undefined;
    let utxo;
    ...
    const extraBytes = this.coin.getExtraBytes({
      floData
    });
    const extraBytesLength = extraBytes.length; // console.log(formattedUtxos)
    // let utxosNoUnconfirmed = formattedUtxos.filter(utx => utx.confirmations > 0) //ToDo
    // console.log(utxosNoUnconfirmed)

    let selected = (0, _coinselect.default)(formattedUtxos, targets, Math.ceil(this.coin.feePerByte), extraBytesLength); // Check if we are able to build inputs/outputs off only unconfirmed transactions with confirmations > 0

    if (!selected.inputs || selected.inputs.length === 0 || !selected.outputs || selected.outputs.length === 0 || !selected.fee) {
      selected = (0, _coinselect.default)(formattedUtxos, targets, Math.ceil(this.coin.feePerByte), extraBytesLength);
    }
  ...
  removeSpent(unspentTransactions) {
    ...
    for (const tx of unspentTransactions) {
      ...
      for (const txid of this.getSpentTransactions()) {
        if (txid === tx.txid) {
          spent = true;
        }
      }
  ...
  createManualUtxos() {
    ...
    for (const txObj of this.history) {
      let match = false;

      for (const tx of this.getSpentTransactions()) {
        for (const txid in txObj) {
          if (txid === tx) {
            match = true;
          }
        }
    ...
    for (const txObj of unspents) {
      for (const txid in txObj) {
        floTxs.push(_tx.default.fromRaw(txObj[txid], 'hex'));
      }
    } // console.log(floTxs)
    ...
    for (const f of floTxs) {
      // console.log(f)
      const outputs = f.outputs;

      for (let i = 0; i < outputs.length; i++) {
        let addr = outputs[i].getAddress(); // console.log(addr)
        ...
        const _address$fromBase58Ch = _bitcoinjsLib.address.fromBase58Check(addr),
              hash = _address$fromBase58Ch.hash;

        const testnetAddr = _bitcoinjsLib.address.toBase58Check(hash, 115);
        ...
        if (testnetAddr === this.p2pkh) {
          const tmpObj = {
            ...
            confirmations: 0 // console.log(tmpObj)
  ...
  async sendTx(outputs) {
    let floData = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
