...
var _FLOTransactionBuilder = _interopRequireDefault(require("../flo/FLOTransactionBuilder"));

var _FLOTransaction = require("../flo/FLOTransaction");

var _Peer = _interopRequireDefault(require("../flo/Peer"));
...
const ONE_MINUTE = 60 * ONE_SECOND; // 1 satoshis per byte (1000 satoshi per kb) (100 million satoshi in 1 FLO)

const TX_FEE_PER_BYTE = 0.00000001; // Average size of tx data (without floData) to calculate min txFee

const TX_AVG_BYTE_SIZE = 200;
const SAT_PER_FLO = 100000000; // The minimum amount a utxo is allowed to be for us to use

const MIN_UTXO_AMOUNT = 0.0001; // Prevent chaining over ancestor limit

const MAX_MEMPOOL_ANCESTORS = 1250;
const MAX_MEMPOOL_ANCESTOR_SIZE = 1.75 * ONE_MB; // Timer lengths used to track and fix the Ancestor chain
...
const REPAIR_MIN_TX = 100; // List of all Wallet-Only RPC Methods

const WALLET_RPC_METHODS = ['selectwallet', 'getwalletinfo', 'fundrawtransaction', 'resendwallettransactions', 'abandontransaction', 'addmultisigaddress', 'addwitnessaddress', 'backupwallet', 'dumpprivkey', 'dumpwallet', 'encryptwallet', 'getaccountaddress', 'getaccount', 'getaddressesbyaccount', 'getbalance', 'getnewaddress', 'getrawchangeaddress', 'getreceivedbyaccount', 'getreceivedbyaddress', 'gettransaction', 'getunconfirmedbalance', 'importprivkey', 'importwallet', 'importaddress', 'importprunedfunds', 'importpubkey', 'keypoolrefill', 'listaccounts', 'listaddressgroupings', 'lockunspent', 'listlockunspent', 'listreceivedbyaccount', 'listreceivedbyaddress', 'listsinceblock', 'listtransactions', 'listunspent', 'move', 'sendfrom', 'sendmany', 'sendtoaddress', 'setaccount', 'settxfee', 'signmessage', 'walletlock', 'walletpassphrasechange', 'walletpassphrase', 'removeprunedfunds'];
/**
...
class RPCWallet {
  constructor(options) {
    ...
    this.options = options || {}; // Set default network to livenet if unset

    if (!this.options.network) {
      this.options.network = 'livenet';
    ...
    this.rpc = _axios.default.create({
      auth: {
    ...
    this.fcoinRPC = false; // Store the Private Key and the Public Key

    this.wif = this.options.wif;
    this.publicAddress = this.options.publicAddress;
    this.importPrivateKey = this.options.importPrivateKey || true; // Store the "coin" network we should use

    if (this.options.network === 'livenet' || this.options.network === 'mainnet' || this.options.network === 'main') {
      this.coin = _config.floMainnet;
    } else if (this.options.network === 'testnet') {
      this.coin = _config.floTestnet;
    } else if (this.options.network === 'regtest') {
      this.coin = _config.floRegtest;
    ...
    this.onConfirmationSubscriptions = {}; // Initialize our initial peer array

    this.peers = []; // Variables to count utxo ancestors (maximum number of unconfirmed transactions you can chain)

    this.currentAncestorCount = 0;
    this.currentAncestorSize = 0; // Repair mode tracker
  ...
  async rpcRequest(method, parameters) {
    ...
    try {
      ...
      let rpcPort = parseInt(this.options.rpc.port); // If we are using an fcoin RPC, the wallet RPC runs on a different port (7315/17315 instead of 7313/17313)

      if (WALLET_RPC_METHODS.includes(method) && this.fcoinRPC) {
        rpcPort += 2;
      } // Attempt the RPC request


      rpcRequest = await this.rpc.post(`http://${this.options.rpc.host}:${rpcPort}/`, {
        jsonrpc: '2.0',
        id: 1,
        method: method,
        params: parameters
      ...
      console.log("[RPC Wallet] Unable to perform RPC request, retrying! Method: '" + method + "' - Params: '" + JSON.stringify(parameters) + "' | RPC settings: " + JSON.stringify(this.options.rpc) + ' | Thrown Error: ' + e); // Sleep 1 second
  ...
  async updateAncestorStatus() {
    console.log('[RPC Wallet] Updating Ancestor Status...'); // We next check to see if there are any transactions currently in the mempool that we need to be aware of.
    // To check the mempool, we start by grabbing the UTXO's to get the txid of the most recent transaction that was sent

    const utxos = await this.getUTXOs(); // Grab the most recent txid

    const mostRecentUTXO = utxos[0];
    const mostRecentTXID = mostRecentUTXO.txid; // Check to see if the utxo is still in the mempool and if it has ancestors

    const getMempoolEntry = await this.rpcRequest('getmempoolentry', [mostRecentTXID]); // Check if we have an error and handle it
    ...
    if (getMempoolEntry.error && getMempoolEntry.error !== null) {
      ...
      if (getMempoolEntry.error.message === 'Transaction not in mempool' || getMempoolEntry.error.message === 'Transaction not in mempool.') {
        ...
        const checkConfirmations = await this.rpcRequest('getrawtransaction', [mostRecentTXID, true]); // Ignore if there was an error, but set the number of confirmations if available
        ...
        if (mostRecentUTXO.confirmations >= 1) {
          ...
          await this.rebroadcastTransactions(); // Don't update anything, and return for now, so that `updateAncestorStatus` will run again
    ...
    if (getMempoolEntry.result) {
      const txMempoolStatus = getMempoolEntry.result; // Store the current ancestor count & size

      this.currentAncestorCount = txMempoolStatus.ancestorcount;
      this.currentAncestorSize = txMempoolStatus.ancestorsize; // Also increase the count by one in order to account for the txid from the mempool
  ...
  addAncestor(hex) {
    ...
    this.currentAncestorCount++; // Increase the ancestor size (byte length)

    this.currentAncestorSize += Buffer.from(hex, 'hex').length; // Log every 50
  ...
  async checkAncestorCount(forceUpdateAncestor) {
    ...
    const startAncestorCount = this.currentAncestorCount;
    const startAncestorSize = this.currentAncestorSize; // Variables to track loop state

    let firstLoop = true;
    let hadMaxAncestors = false;
    let reachedAncestorLimitTimestamp; // Check if we have too many ancestors, and if we do, wait for the ancestor count to decrease (aka, some transactions to get confirmed in a block)
    ...
    while (this.currentAncestorCount >= MAX_MEMPOOL_ANCESTORS || this.currentAncestorSize >= MAX_MEMPOOL_ANCESTOR_SIZE || forceFirstLoopUpdate) {
      ...
      forceFirstLoopUpdate = false; // Wait for UPDATE_ANCESTOR_STATUS seconds (don't run on the first loop through)

      if (!firstLoop) {
        await new Promise((resolve, reject) => {
          setTimeout(() => {
            resolve();
          }, UPDATE_ANCESTOR_STATUS);
        });
      } // Update the ancestor status (this is what will break us out of our while loop)


      await this.updateAncestorStatus(); // Check if we currently have the maximum number of ancestors

      const hasMaxAncestors = this.currentAncestorCount >= MAX_MEMPOOL_ANCESTORS || this.currentAncestorSize >= MAX_MEMPOOL_ANCESTOR_SIZE; // Only log ancestor count if it is the first loop, and we still have too many ancestors
      ...
      if (hasMaxAncestors && Date.now() - REPAIR_ANCESTORS_AFTER > reachedAncestorLimitTimestamp) {
        ...
        console.log(`[RPC Wallet] [WARNING] Unconfirmed Transaction count has not decreased in ${REPAIR_ANCESTORS_AFTER / ONE_MINUTE} minutes, rebroadcasting transactions in an attempt to repair the utxo chain!`); // Attempt the actual repair

        await this.rebroadcastTransactions(); // Reset the timestamp so that we wait for REPAIR_ANCESTORS_AFTER seconds/minutes
    ...
    const numberConfirmed = startAncestorCount - this.currentAncestorCount; // Remove the transactions that just got confirmed
    ...
    if (startAncestorCount >= MAX_MEMPOOL_ANCESTORS || startAncestorSize >= MAX_MEMPOOL_ANCESTOR_SIZE) {
      console.log(`[RPC Wallet] ${numberConfirmed} Transactions Confirmed! (${((startAncestorSize - this.currentAncestorSize) / ONE_MB).toFixed(2)} MB)`); // If there are less confirmed than REPAIR_MIN_TX, then rebroadcast the transactions
    ...
    this.repairMode = false; // There are fewer ancestors than the maximum, so we can send the next transaction!
  ...
  async rebroadcastTransactions() {
    ...
    console.log(`[RPC Wallet] Announcing ${this.unconfirmedTransactions.length} transactions to Peers...`); // Connect to Peers to use for the rebroadcast

    await this.connectToPeers(); // Announce all of our local unconfirmed transactions

    let numAnnounced = 0;

    for (const txHex of this.unconfirmedTransactions) {
    ...
    console.log(`[RPC Wallet] Announced ${numAnnounced} transactions!`); // Wait for REBROADCAST_LENGTH in order to give some time for transactions to be requested, and sent out.
  ...
  async initialize() {
    if (this.importPrivateKey) {
      console.log('[RPC Wallet] Importing the Private Key to the RPC Wallet, this may take a long time...'); // First, we import the Private Key to make sure it exists when we attempt to send transactions.

      const importPrivKey = await this.rpcRequest('importprivkey', [this.wif, 'default', true]);
      console.log('[RPC Wallet] Private Key Import to the RPC Wallet Complete!'); // Check for an error importing the private key. If there is no error, the private key import was successful.
    ...
    await this.updateAncestorStatus(); // If we have no ancestors (i.e. if our tx already got confirmed), skip grabbing the ancestor transaction hex and return true early
    ...
    const utxos = await this.getUTXOs(); // Then, while we have ancestors, lookup the transaction hex, and add it to the start of the unconfirmed transaction chain

    let nextTXID = utxos[0].txid;

    while (nextTXID) {
      // Grab the raw transaction hex for the txid
      const txHex = await this.rpcRequest('getrawtransaction', [nextTXID]);
      ...
      this.unconfirmedTxids.unshift(nextTXID); // Then lookup to see if it is in the mempool

      const txMemInfo = await this.rpcRequest('getmempoolentry', [nextTXID]);
    ...
    console.log(`[RPC Wallet] Loaded ${this.unconfirmedTransactions.length} transactions into local mempool!`); // Return true, signifying that the initialization was successful
  ...
  async connectToPeers() {
    ...
    this.peers = []; // Request peers to connect to from the RPC node we have access to

    const getPeerInfo = await this.rpcRequest('getpeerinfo', []);

    if (getPeerInfo.error) {
      throw new Error(getPeerInfo.error);
    }

    console.log(`[RPC Wallet] Connecting to ${getPeerInfo.result.length} Peers...`); // For each of those peers, open up a connection

    for (const peerInfo of getPeerInfo.result) {
      // Create an fcoin peer
      let peerHost = peerInfo.addr; // Always rewrite the port.
      ...
      const peer = new _Peer.default({
        ip: peerHost,
        network: this.options.network
      }); // Start the connection attempt

      peer.connect(); // Add it to our peerrs array
  ...
  async signMessage(message) {
    ...
    const myECPair = _bitcoinjsLib.ECPair.fromWIF(this.wif, this.coin.network);

    const privateKeyBuffer = myECPair.privateKey; // Check if we are a compress privKey

    const compressed = myECPair.compressed || true; // Create the actual signature

    let signatureBuffer;

    try {
      signatureBuffer = (0, _bitcoinjsMessage.sign)(message, privateKeyBuffer, compressed, myECPair.network.messagePrefix);
    } catch (e) {
      throw new Error(e);
    } // Convert the signature to a base64 string


    const signature = signatureBuffer.toString('base64'); // Return the base64 signature
  ...
  async getUTXOs() {
    ...
    const utxoRes = await this.rpcRequest('listunspent', [MIN_CONFIRMATIONS, MAX_CONFIRMATIONS, [this.publicAddress]]); // Throw if there was an error
    ...
    const utxos = utxoRes.result; // Filter out transactions that don't meet our minimum UTXO value
  ...
  async sendDataToChain(floData) {
    ...
    const utxos = await this.getUTXOs(); // Select the first input (since we have already sorted and filtered)

    const input = utxos[0]; // Calculate the minimum Transaction fee for our transaction by counting the size of the inputs, outputs, and floData

    const myTxFee = (this.options.txFeePerByte || TX_FEE_PER_BYTE) * (TX_AVG_BYTE_SIZE + (0, _util.varIntBuffer)(floData.length).toString('hex').length + Buffer.from(floData).length); // Create an output to send the funds to

    const output = {};
    output[this.publicAddress] = parseFloat((input.amount - myTxFee).toFixed(8)); // Send the transaction

    const txid = await this.sendTX([input], output, floData); // Returns the TXID of the transaction if sent successfully
  ...
  createAndSignTransaction(input, outputs, floData) {
    ...
    const txb = new _FLOTransactionBuilder.default(this.coin.network); // Add our single input

    txb.addInput(input.txid, input.vout); // Add our output

    txb.addOutput(this.publicAddress, parseInt(outputs[this.publicAddress] * SAT_PER_FLO)); // Add our floData

    txb.setFloData(floData); // Sign our transaction using the local `flosigner` at `src/config/networks/flo/flosigner.js`

    txb.sign(0, _bitcoinjsLib.ECPair.fromWIF(this.wif, this.coin.network)); // Build the hex
  ...
  async sendTX(inputs, outputs, floData) {
    ...
    if (floData.length > _FLOTransaction.FLODATA_MAX_LEN) {
      throw new Error(`Error: 'floData' length exceeds ${_FLOTransaction.FLODATA_MAX_LEN} characters. Please send a smaller data package.`);
    } // Make sure that we don't have too many ancestors. If we do, then waits for some transactions to be confirmed.


    await this.checkAncestorCount(); // Check if transactions have been confirmed, and if so run the callback provided

    await this.checkForConfirmations(); // Create and sign the transaction hex

    const signedTXHex = this.createAndSignTransaction(inputs[0], outputs, floData); // Broadcast the transaction hex we created to the network

    const broadcastTX = await this.rpcRequest('sendrawtransaction', [signedTXHex]); // Check if there was an error broadcasting the transaction

    if (broadcastTX.error && broadcastTX.error !== null) {
      throw new Error('Error broadcasting tx: ' + signedTXHex + '\n' + JSON.stringify(broadcastTX.error));
    } // Add the tx we just sent to the Ancestor count


    this.addAncestor(signedTXHex); // Add our tx hex to the unconfirmed transactions array

    this.unconfirmedTransactions.push(signedTXHex);
    this.unconfirmedTxids.push(broadcastTX.result); // Set the new tx to be used as the next output.

    this.previousTXOutput = {
      txid: broadcastTX.result,
      amount: outputs[this.publicAddress],
      vout: 0 // Return the TXID of the transaction
  ...
  onConfirmation(_ref, options) {
    let txids = _ref.txids,
        record = _ref.record;

    // If we do not have an onConfirmation subscription, exit the function
    if (!options || !options.onConfirmation || typeof options.onConfirmation !== 'function') {
      return;
  ...
  async checkForConfirmations() {
    for (const subscription in this.onConfirmationSubscriptions) {
      const _this$onConfirmationS = this.onConfirmationSubscriptions[subscription],
            txids = _this$onConfirmationS.txids,
            record = _this$onConfirmationS.record,
            options = _this$onConfirmationS.options; // Check if we are still waiting to be confirmed
  ...
  async waitForConfirmations() {
    ...
    while (subCount > 0) {
      ...
      await new Promise((resolve, reject) => {
        setTimeout(resolve, CONFIRMATION_CHECK_LENGTH);
      });
      await this.checkAncestorCount(true);
      await this.checkForConfirmations();
      const tmpSubCount = subCount;
