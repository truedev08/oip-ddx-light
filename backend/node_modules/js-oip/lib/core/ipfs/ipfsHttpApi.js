...
class IpfsHttpApi {
  ...
  async start() {
    ...
    const onProgressFunc = bytesAdded => {
      const progressObject = {
        bytes_uploaded: bytesAdded
      };

      if (this.options.filesize) {
        const percent = parseFloat((bytesAdded / this.options.filesize * 100).toFixed(2));
        progressObject.uploadProgress = percent;
        progressObject.bytesTotal = this.options.filesize;
      }

      for (const progressFunction of this.progressSubscribers) {
