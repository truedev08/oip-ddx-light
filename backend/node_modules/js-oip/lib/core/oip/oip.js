...
class OIP {
  ...
  constructor(wif, network, options) {
    ...
    if (!this.options.publicAddress) {
      ...
      const myECPair = _bitcoinjsLib.ECPair.fromWIF(this.options.wif, tmpNetwork.network);
  ...
  async signRecord(record) {
    ...
    if (!record.getSignature() || record.getSignature() === '') {
      ...
      const _ref = await record.signSelf(this.wallet.signMessage.bind(this.wallet)),
  ...
  async broadcastRecord(record, methodType, options) {
    ...
    const _record$isValid = record.isValid(),
    ...
    const broadcastString = record.serialize(methodType); // Array to store txids
    ...
    if (broadcastString.length > _FLOTransaction.FLODATA_MAX_LEN) {
      ...
      try {
        ...
        const txid = await this.wallet.sendDataToChain(broadcastString);
        txids = [txid];
      } catch (err) {
        return {
          success: false,
    ...
    if (record instanceof _edit.EditRecord) {
      ...
      const patchedRecord = record.getPatchedRecord(); // Set the edit version to the EditRecord txid
  ...
  async publish(record, options) {
    ...
    const res = await this.broadcastRecord(record, 'publish', options);
    return res;
  ...
  async edit(editedRecord) {
    ...
    try {
      const _ref2 = await this.oipdAPI.getRecord(editedRecord.getOriginalTXID()),
    ...
    if (!original) {
      return {
        success: false,
        error: `A Record with the txid ${editedRecord.getOriginalTXID()} was not found in OIP daemon! Please make sure you have set 'options.oipdURL' to your OIP daemon server!`
      };
    ...
    const edit = new _edit.EditRecord(undefined, original, editedRecord); // Publish to chain

    const res = await this.broadcastRecord(edit, 'edit');
    return res;
  ...
  async publishMultiparts(data) {
    ...
    const mpx = new _modules.MultipartX(data);
    ...
    for (const mp of mps) {
      ...
      const _ref3 = await mp.signSelf(this.wallet.signMessage.bind(this.wallet)),
            error = _ref3.error;
