...
var _coinselect = _interopRequireDefault(require("@oipwg/coinselect"));
...
class TransactionBuilder {
  ...
  async buildInputsAndOutputs(manualUtxos) {
    ...
    if (this.coin.network.hasFloData) {
      extraBytesLength = this.passedOptions.floData ? this.passedOptions.floData.length : 0;
    }

    const utxosNoUnconfirmed = formattedUtxos.filter(utx => utx.confirmations > 0);
    let selected = (0, _coinselect.default)(utxosNoUnconfirmed, targets, Math.ceil(this.coin.feePerByte), this.coin.minFee, extraBytesLength); // Check if we are able to build inputs/outputs off only unconfirmed transactions with confirmations > 0

    if (selected.inputs && selected.inputs.length > 0 && selected.outputs && selected.outputs.length > 0 && selected.fee) {// return selected
    } else {
      // else, build with the regular ones
      selected = (0, _coinselect.default)(formattedUtxos, targets, Math.ceil(this.coin.feePerByte), this.coin.minFee, extraBytesLength);
    }
  ...
  async buildTX(manualSelected) {
    ...
    let outputs = selected.outputs; // inputs and outputs will be undefined if no solution was found
    ...
    if (this.coin.hasFloData === true) {
      ...
      txb = new _FloTransaction.FloPsbt({
        network: this.coin.network
      });
      txb.setFloData(floData);
    } else {
      txb = new bitcoin.Psbt({
        network: this.coin.network
      });
    }
    ...
    for (const addr of this.from) {
      ...
      } catch {// sign throws if there is no input to be signed by addr
      }
    }
